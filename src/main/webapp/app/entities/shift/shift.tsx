  ...
  return (
    <div>
      <h2 id="shift-heading" data-cy="ShiftHeading">
        <Translate contentKey="rosterSampleApplicationApp.shift.home.title">Shifts</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="rosterSampleApplicationApp.shift.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/shift/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="rosterSampleApplicationApp.shift.home.createLabel">Create new Shift</Translate>
          </Link>
        </div>
      </h2>
        ...
        {shiftList && shiftList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="rosterSampleApplicationApp.shift.id">ID</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('key')}>
                  <Translate contentKey="rosterSampleApplicationApp.shift.key">Key</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('key')} />
                </th>
                <th className="hand" onClick={sort('shiftStart')}>
                  <Translate contentKey="rosterSampleApplicationApp.shift.shiftStart">Shift Start</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('shiftStart')} />
                </th>
                <th className="hand" onClick={sort('shiftEnd')}>
                  <Translate contentKey="rosterSampleApplicationApp.shift.shiftEnd">Shift End</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('shiftEnd')} />
                </th>
                <th className="hand" onClick={sort('type')}>
                  <Translate contentKey="rosterSampleApplicationApp.shift.type">Type</Translate>{' '}
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="rosterSampleApplicationApp.shift.home.notFound">No Shifts found</Translate>
            </div>
          )
        )}
      </div>
